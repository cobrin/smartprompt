...
function parse_git_branch {
  ...
  if [[ ${git_status} =~ ${remote_pattern} ]]; then
    ...
    #remote="${BOLD_YELLOW}\[$(tput sc)\] \[$(tput rc)\]\[↓\]"
...
function prompt_func() {
    ...
    # default prompt with timestamp
    prompt="[${GREEN}\t${COLOR_NONE}|"
    ...
    then : "VIRTUAL_ENV=$VIRTUAL_ENV"
        ...
        #prompt="${prompt}${BOLD_YELLOW}${UNAME}@$(hostname)${BOLD_GREEN}$(parse_git_branch)${TITLEBAR}"
        #prompt="${prompt}${BOLD_BLUE}[${BOLD_RED}\w${BOLD_BLUE}]${COLOR_NONE}"
        if [ $(id -u) -eq 0 ]
        then : "root user"
           # root settings
           prompt="${prompt}${RED}\u${COLOR_NONE}@${BOLD_RED}\h${COLOR_NONE}"
        else : "non-root user"
           # non-root settings
           prompt="${prompt}${CYAN}\u${COLOR_NONE}@${BOLD_CYAN}\h${COLOR_NONE}"
        fi
